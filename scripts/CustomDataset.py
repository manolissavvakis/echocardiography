...
class CustomImageDataset(Dataset):
    def __init__(self, path: str, transform: bool):
        self.path = path
        self.training_data = pd.read_csv(Path.joinpath(self.path, 'train.csv'), usecols = ['patient_id','pathological_risk_label'])
        self.validation_data = pd.read_csv(Path.joinpath(self.path, 'val.csv'), usecols = ['patient_id','pathological_risk_label'])
    ...
    def __getitem__(self, idx):
        ...
        image = sitk.ReadImage(Path.cwd().joinpath("data", "database_nifti", patient_dir, patient_file))
