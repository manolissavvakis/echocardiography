...
def train(model, epoch, batch_size, training_set, device):
    ...
    print(f"Average loss of epoch {epoch+1}, using {batch_idx+1} batches: {running_loss / (batch_idx+1)}")
...
def validation(model, epoch, batch_size, validation_set, device):
    ...
    with torch.no_grad():
        ...
        print(f"Average validation loss of epoch {epoch+1}, using {total} images: {avg_loss}")
...
def main():
    ...
    for fold in Path.cwd().joinpath('data', quality[0], 'folds').glob('*'):
        ...
        for epoch in range(epochs):
            ...
            # Validation step
            avg_loss, accuracy = validation(model, epoch, batch_size, val_dataset, device)

            # Save the best model.
            if avg_loss < best_loss:
                best_loss = model_saving(model, fold.stem[-1], epoch, **{'avg_loss': avg_loss, 'accuracy': accuracy})
                best_loss = avg_loss
